...
RegionECD = zeros(1, RegionNo);
 newClass=[1 1 1 1 0 0 0 0]';
            svmStruct =  svmtrain(double(Im(1:length(newClass))),newClass,'Kernel_Function','rbf','Method','QP');
        yl = svmclassify(svmStruct,tst);
...
ylabel('Accuracy (%)')
legend('Gabor Filter','2D Box Filter')
